...
- **Manifest builders** (`packages/shared/src/openshift/manifests.ts`) — dependency-free, unit-tested builders that emit the `Deployment` / `Service` / `Route` objects for one simulation pod:
  - Deployment: `ENTRYPOINT /bin/bash` with `/entrypoint-gazebo.sh` arg, CPU/software rendering only (`LIBGL_ALWAYS_SOFTWARE=1`, `GALLIUM_DRIVER=llvmpipe` — **no GPU in-cluster**), **4 guaranteed CPUs** (`requests == limits`, needed for real-time under software rendering — see CPU-sizing note below), noVNC on container port 6080, TCP readiness probe.
...
**In-cluster Navigate hang → CPU sizing (2026-08-14):** After the segfault fix, spawning worked but **Navigate hung on "Navigating…"** with the robot not moving. The Nav2 stack was fully healthy (robot localized, `navigate_to_pose` accepted, controller publishing `cmd_vel`) — the real problem was **CPU starvation**: on a 2-core pod, software rendering pins the sim's real-time factor at **~0.01–0.14**, so the robot crawls and the goal never finishes inside the 180s timeout. Sweep on `sgahlot-pd-extn`: **2 CPUs → RTF ~0.1 → hang; 4 CPUs → RTF ~1.0 → `SUCCEEDED`**. The software-rendering Deployment now requests **4 guaranteed CPUs** (`requests == limits`); the GPU path stays at 2 (render load moves off-CPU). The CPU hog is the `gz sim -g` GUI (~2.3 cores rendering the scene for noVNC), **not** the depth camera, so dropping sensors would not lower the floor — a GPU would. RTF is capped at real-time, so 6/8 cores add only multi-robot headroom.
