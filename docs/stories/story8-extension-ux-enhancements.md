...
| ✅ | S8-18 | Prune stale robot entries | **Done** (APPENG-6149). Found during APPENG-6148 testing (2026-08-20): a pod crashed/restarted mid-spawn (Path B always resets to an empty world), but the UI kept showing the robot as spawned — "Nav2 warming…" → "Nav2 warm-up failed" → Navigate "Failed" — since nothing told it the spawn attempt died with the pod. S8-17 only handles the ADD direction (by design — "only ever appends missing entries, never removes"); this is the missing REMOVE/prune direction. New `pruneStaleRobots()` runs on every 3s poll tick (unlike the once-per-workload `reconcileRobots`, since the crash can happen well after that one-time reconcile) and is **phase-aware**: a robot with `warmStatus === 'warming'` is skipped entirely (its own state machine, backed by the backend's bounded pre-warm timeout, already owns resolving that); a freshly-spawned robot with no `warmStatus` signal (Humble) gets a 30s wall-clock grace period for raw spawn-to-registration latency; otherwise a robot must be confirmed missing across 2 consecutive polls (debounced, since `listSpawnedRobotsInOpenShift` never throws and can't distinguish a genuinely empty world from a transient exec hiccup) before being dropped. Live-tested against a real OpenShift pod restart (`sgahlot-pd-extn`) — the phantom robot correctly disappeared once the replacement pod became ready. | `OpenShiftSimulation.svelte` (`pruneStaleRobots`, `missingStreaks`, `trackedSince`), `OpenShiftSimulation.spec.ts` (5 new tests) |
