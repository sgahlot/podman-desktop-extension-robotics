...
| [APPENG-5766](#story-3) | Multi-robot local scaling | ⚪ Not Started | 5775 **Closed** (single-container/pod `rmw_zenoh_cpp` foundation landed, tested locally + OpenShift; cross-container fleet win awaits 5774 Compose topology); 5774/5776 New |
...
| 🟠 | APPENG-5920 | Add navigation UI for driving robots in simulation | **In Review.** Per-robot X/Y + **Navigate** on Simulation page. Jazzy: Nav2 `navigate_to_pose` (5981); Humble: `cmd_vel` fallback. |
...
| ✅ | APPENG-5775 | Native `rmw_zenoh_cpp` middleware, selectable at runtime | **Closed** — `rmw_zenoh_cpp` baked into `ros2-jazzy-sim` alongside DDS, selected via `RMW_IMPLEMENTATION`, router (`rmw_zenohd`) runs as a background daemon — tested working single-container (Podman) and single-pod (OpenShift), including a live Nav2 navigation goal over Zenoh discovery. Cross-container/cross-pod fleet win (shared router across separate containers/pods) still needs 5774's Compose topology (here) and the multi-pod split ([story7](stories/story7-multipod-openshift-architecture.md) / APPENG-6070). |
...
| **🟠 Story 2 sub-tasks In Review** | APPENG-5920 + 5922 + 5923 | Navigate UI (Jazzy Nav2 / Humble cmd_vel), Topic drill-down, Peek — pending review |
...
✅ APPENG-5768 (Scaffold) ── DONE, unblocks everything below
    ...
    ├── 🟠 APPENG-5920 (Navigate UI) ── IN REVIEW (5981 wires Jazzy Nav2)
    │
...
| ✅ | APPENG-5775 | Sub-task | APPENG-5766 | Native `rmw_zenoh_cpp` middleware, selectable at runtime (single-container/pod foundation landed, Closed) |
...
| F — Secure base images → wizard | Hummingbird/bootc feasibility spike (NO-GO natively today), then a layer-composition wizard with a real pull + build path | ✅ Done — merged to `main` (`0b94ed0`) |
