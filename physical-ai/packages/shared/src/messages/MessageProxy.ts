...
export class RpcExtension {
  ...
  register(channel: string, method: (...args: unknown[]) => Promise<unknown>, maxArgs: number = method.length): void {
    this.methods.set(channel, async (...args: unknown[]) => {
      ...
      if (maxArgs > 0 && args.length > maxArgs) {
        throw new Error(`Too many arguments for ${channel}: got ${args.length}, expected at most ${maxArgs}`);
      }
      return method(...args);
    });
  }
...
export class RpcBrowser {
  ...
  subscribers: Map<string, (msg: unknown) => void> = new Map();
  ...
  subscribe(msgId: string, f: (msg: unknown) => void): Subscriber {
  ...
  isSubscribedMessage(content: unknown): content is ISubscribedMessage {
    return (
      ...
      'id' in content &&
      'body' in content &&
      typeof (content as ISubscribedMessage).id === 'string' &&
      this.subscribers.has((content as ISubscribedMessage).id)
    );
  }
}
