...
**In-cluster CPU sizing (no GPU):** software rendering is CPU-bound — the `gz sim -g` GUI client alone needs ~2.3 cores to render the scene for noVNC. On a 2-core pod the sim's real-time factor collapses to ~0.1, so **Navigate** drives the robot but it crawls and the goal times out (UI stuck on "Navigating…"). The software-rendering Deployment therefore requests **4 guaranteed CPUs** (`requests == limits`), which restores real-time (RTF ~1.0) and lets Nav2 goals complete. RTF is capped at real-time, so more than 4 cores only adds headroom for multi-robot runs; and because the bottleneck is the GUI (not the depth camera), dropping sensors doesn't lower the requirement — a GPU does.
